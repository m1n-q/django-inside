...
def init(request):
	msg = psy.create_movies_table(table='ex04_movies')
	return render(request, 'ex04/init.html', context={'msg': msg} )
...
def populate(request):
	...
	for d in data:
		...
		result.append(msg)
	return render(request, 'ex04/populate.html', context={'result': result} )


def display(request):
	records = psy.select_all_from_table(table='ex04_movies')
	if type(records) == str:
		msg = records
		return render(request, 'ex04/display.html', context={'msg': msg} )

	return render(request, 'ex04/display.html', context={'records': records} )
...
def remove(request):
	...
	return render(request, 'ex04/remove.html', context={'form' : form, 'msg' : msg})
